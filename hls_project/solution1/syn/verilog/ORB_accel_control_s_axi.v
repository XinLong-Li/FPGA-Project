...
// ==============================================================
`timescale 1ns/1ps
module ORB_accel_control_s_axi
#(parameter
    ...
    output wire [31:0]                   height_new,
    output wire [31:0]                   width_new,
...
localparam
    ...
    ADDR_HEIGHT_NEW_DATA_0 = 6'h28,
    ADDR_HEIGHT_NEW_CTRL   = 6'h2c,
    ADDR_WIDTH_NEW_DATA_0  = 6'h30,
    ADDR_WIDTH_NEW_CTRL    = 6'h34,
    ADDR_THRESHOLD_DATA_0  = 6'h38,
    ADDR_THRESHOLD_CTRL    = 6'h3c,
    WRIDLE                 = 2'd0,
    WRDATA                 = 2'd1,
    WRRESP                 = 2'd2,
    WRRESET                = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_height_new = 'b0;
    reg  [31:0]                   int_width_new = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_HEIGHT_NEW_DATA_0: begin
                    rdata <= int_height_new[31:0];
                end
                ADDR_WIDTH_NEW_DATA_0: begin
                    rdata <= int_width_new[31:0];
                end
...
assign height_new = int_height_new;
assign width_new  = int_width_new;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_height_new[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_HEIGHT_NEW_DATA_0)
            int_height_new[31:0] <= (WDATA[31:0] & wmask) | (int_height_new[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_width_new[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_WIDTH_NEW_DATA_0)
            int_width_new[31:0] <= (WDATA[31:0] & wmask) | (int_width_new[31:0] & ~wmask);
    end
end
